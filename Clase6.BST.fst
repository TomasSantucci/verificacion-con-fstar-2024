...
let rec all_lt' (x:int) (y:int{x>y}) (t:bst) : Lemma (requires all_lt y t) (ensures not (in_tree x t)) =
  match t with
  | L -> ()
  | N (l,z,r) -> (
    all_lt' x y l;
...
let rec all_gt' (x:int) (y:int{x<y}) (t:bst) : Lemma (requires all_gt y t) (ensures not (in_tree x t)) =
  match t with
  | L -> ()
  | N (l,z,r) -> (
    all_gt' x y l;
